        ...
        <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
            <div class="flex flex-col items-center justify-between space-y-3 p-4 md:flex-row md:space-x-4 md:space-y-0">
                ...
                <div class="flex w-full flex-shrink-0 flex-col items-stretch justify-end space-y-2 md:w-auto md:flex-row md:items-center md:space-x-3 md:space-y-0">
                    @can('create a pool')
                        <x-link class="bg-primary-700 text-white hover:bg-primary-800 focus:ring-primary-300" href="{{ route('pool.create') }}">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-500">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-700">
                        <tr>
                            <th class="px-4 py-3" scope="col">pool name</th>
                            <th class="px-4 py-3" scope="col">location</th>
                            @canany(['edit a pool', 'delete a pool'])
                    ...
                    <tbody>
                        @forelse ($pools as $pool)
                            <tr class="border-b">
                                <th class="whitespace-nowrap px-4 py-3 font-medium text-gray-900" scope="row">{{ $pool->name }}</th>
                                <td class="px-4 py-3">{{ $pool->location }}</td>
                                @can('edit a pool')
                                    <td class="flex items-center px-4 py-3">
                                        <x-link class="border border-primary-700 text-primary-700 hover:text-primary-800" href="{{ route('pool.edit', $pool) }}">
                                    ...
                                    @endcan
                                    @can('delete a pool')
                                        <form class="inline-block px-2" method="POST" action="{{ route('pool.destroy', $pool) }}">
                                            @csrf
                                            @method('DELETE')
