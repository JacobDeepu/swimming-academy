        ...
        <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
            <div class="flex flex-col items-center justify-between space-y-3 p-4 md:flex-row md:space-x-4 md:space-y-0">
                ...
                <div class="flex w-full flex-shrink-0 flex-col items-stretch justify-end space-y-2 md:w-auto md:flex-row md:items-center md:space-x-3 md:space-y-0">
                    @can('create a user')
                        <x-link class="bg-primary-700 text-white hover:bg-primary-800 focus:ring-primary-300" href="{{ route('user.create') }}">
                        ...
                        @forelse ($users as $user)
                            <tr class="border-b">
                                ...
                                @can('edit a user')
                                    ...
                                    @endcan
                                    @can('delete a user')
                                        <form class="inline-block px-2" method="POST" action="{{ route('user.destroy', $user) }}">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="p-2">
                {{ $users->appends(request()->query())->links('components.paginator') }}
            </div>
        </div>
    </div>
</x-app-layout>
