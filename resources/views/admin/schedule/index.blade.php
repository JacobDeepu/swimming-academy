<x-app-layout>
    <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg">
            <div class="flex flex-col items-center justify-between space-y-3 p-4 md:flex-row md:space-x-4 md:space-y-0">
                <div class="w-full md:w-1/2">
                    <form class="flex items-center">
                        ...
                        <div class="relative w-full">
                            ...
                            <input class="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2 pl-10 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500" id="simple-search"
                ...
                <div class="flex w-full flex-shrink-0 flex-col items-stretch justify-end space-y-2 md:w-auto md:flex-row md:items-center md:space-x-3 md:space-y-0">
                    @can('create a schedule')
                        <x-link class="bg-primary-700 text-white hover:bg-primary-800 focus:ring-primary-300" href="{{ route('schedule.create') }}">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm text-gray-500">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-700">
                        <tr>
                            <th class="px-4 py-3" scope="col">start time</th>
                            <th class="px-4 py-3" scope="col">end time</th>
                            @canany(['edit a schedule', 'delete a schedule'])
                    ...
                    <tbody>
                        @forelse ($schedules as $schedule)
                            <tr class="border-b">
                                <th class="whitespace-nowrap px-4 py-3 font-medium text-gray-900" scope="row">{{ $schedule->start_time }}</th>
                                <td class="whitespace-nowrap px-4 py-3 font-medium text-gray-900">{{ $schedule->end_time }}</td>
                                @can('edit a schedule')
                                    <td class="flex items-center px-4 py-3">
                                        <x-link class="border border-primary-700 text-primary-700 hover:text-primary-800" href="{{ route('schedule.edit', $schedule) }}">
                                    ...
                                    @can('delete a schedule')
                                        <form class="inline-block px-2" method="POST" action="{{ route('schedule.destroy', $schedule) }}">
                                            @csrf
                                            @method('DELETE')
            ...
            <div class="p-2">
                {{ $schedules->appends(request()->query())->links('components.paginator') }}
            </div>
        </div>
    </div>
</x-app-layout>
